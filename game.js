class DiaDeLosMuertosGame {
  constructor() {
    ...
    this.levels = [
      ...
      { level: 9, targetScore: 5500, moves: 13, description: "Nivel Divino" },
      {
        ...
        targetScore: 7000,
        moves: 12,
        description: "Maestro de los Muertos",
      },
    ];
  ...
  async loadAssets() {
    for (let i = 0; i < this.symbolNames.length; i++) {
      ...
      this.symbolImages[i] = await svgToCanvas(
        SVG_ASSETS[symbolName],
        this.tileSize - 10
      );
    }
    ...
    this.symbolImages["special_cat"] = await svgToCanvas(
      SVG_ASSETS.catSymbol,
      this.tileSize - 10
    );

    // Cargar item especial de fantasma para patrones L y T
    this.symbolImages["special_ghost"] = await svgToCanvas(
      SVG_ASSETS.ghostSymbol,
      this.tileSize - 10
    );
  }
  ...
  animateSkullEyes() {
    const leftEyes = this.leftSkullSvg.querySelectorAll(
      'circle[fill="#ff6b35"]'
    );
    const rightEyes = this.rightSkullSvg.querySelectorAll(
      'circle[fill="#9c27b0"]'
    );
  ...
  handleTileClick(row, col) {
    if (!this.selectedTile) {
      ...
      if (
        selectedTileData.isSpecial &&
        selectedTileData.specialType === "cross"
      ) {
        ...
        clickedTileData.isSpecial &&
        clickedTileData.specialType === "cross"
      ) {
        ...
        selectedTileData.isSpecial &&
        selectedTileData.specialType === "cat"
      ) {
        // Activar poder especial de gato cuando se intercambia con cualquier tile
        this.activateCatPower(selectedRow, selectedCol, row, col);
        this.selectedTile = null;
        return;
      } else if (
        clickedTileData.isSpecial &&
        clickedTileData.specialType === "cat"
      ) {
        // Activar poder especial de gato cuando se intercambia con cualquier tile
        this.activateCatPower(row, col, selectedRow, selectedCol);
        this.selectedTile = null;
        return;
      } else if (
        selectedTileData.isSpecial &&
        selectedTileData.specialType === "ghost"
      ) {
        // Activar poder especial de fantasma cuando se intercambia con cualquier tile
        this.activateGhostPower(selectedRow, selectedCol, row, col);
        this.selectedTile = null;
        return;
      } else if (
        clickedTileData.isSpecial &&
        clickedTileData.specialType === "ghost"
      ) {
        // Activar poder especial de fantasma cuando se intercambia con cualquier tile
        this.activateGhostPower(row, col, selectedRow, selectedCol);
        this.selectedTile = null;
        return;
      }
  ...
  swapTiles(row1, col1, row2, col2) {
    ...
    setTimeout(() => {
      ...
      const allSpecialPatterns = [
        ...(lineResult.specialPatterns || []),
  ...
  findMatches() {
    ...
    for (let row = 0; row < this.gridSize; row++) {
      ...
      for (let col = 1; col < this.gridSize; col++) {
        if (this.board[row][col].type === currentSymbol) {
          ...
          if (count >= 3) {
            // Detectar línea de exactamente 4 para crear item especial de gato
            if (count === 4) {
              const endCol = col - 1; // Final de la línea
              specialPatterns.push({
                type: "bomb_4_horizontal",
                centerPosition: { row, col: endCol },
                tiles: [],
              });
            }
      ...
      if (count >= 3) {
        // Detectar línea de exactamente 4 para crear item especial de gato
        if (count === 4) {
          const endCol = this.gridSize - 1; // Final de la línea (final del tablero)
          specialPatterns.push({
            type: "bomb_4_horizontal",
            centerPosition: { row, col: endCol },
            tiles: [],
    ...
    for (let col = 0; col < this.gridSize; col++) {
      ...
      for (let row = 1; row < this.gridSize; row++) {
        if (this.board[row][col].type === currentSymbol) {
          ...
          if (count >= 3) {
            // Detectar línea de exactamente 4 para crear item especial de gato
            if (count === 4) {
              const endRow = row - 1; // Final de la línea
              specialPatterns.push({
                type: "bomb_4_vertical",
                centerPosition: { row: endRow, col },
                tiles: [],
              });
            }
            // Detectar línea de 5+ para crear item especial de cruz
      ...
      if (count >= 3) {
        // Detectar línea de exactamente 4 para crear item especial de gato
        if (count === 4) {
          const endRow = this.gridSize - 1; // Final de la línea (final del tablero)
          specialPatterns.push({
            type: "bomb_4_vertical",
            centerPosition: { row: endRow, col },
            tiles: [],
  ...
  checkTPatterns(row, col, type) {
    ...
    if (
      this.isValidTPattern(row, col, type, [-1, 0], [1, 0], [0, -1], [0, 1])
    ) {
      const tiles = this.getTPatternTiles(
        row,
        col,
        [-1, 0],
        [1, 0],
    ...
    // ⊤ Upside-down T
    if (
      this.isValidTPattern(row, col, type, [-1, 0], [0, 0], [0, -1], [0, 1])
    ) {
      const tiles = this.getTPatternTiles(
        row,
        col,
    ...
    // ⊣ Left T
    if (
      this.isValidTPattern(row, col, type, [0, -1], [0, 0], [-1, 0], [1, 0])
    ) {
      const tiles = this.getTPatternTiles(
        row,
        col,
  ...
  isValidLPattern(row, col, type, dr1, dc1, dr2, dc2) {
    ...
    for (let i = 1; i < 4; i++) {
      // Start from 1 to avoid double-counting corner
  ...
  getLPatternTiles(row, col, dr1, dc1, dr2, dc2) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (
        this.isValidPosition(newRow, newCol) &&
        this.board[newRow][newCol].type === this.board[row][col].type
      ) {
  ...
  processMatches(matches, patterns = []) {
    ...
    matches.forEach((match) => {
      ...
      for (let i = 0; i < 12; i++) {
        this.particles.push({
          ...
          color: this.colors[tile.type] || "#ff6b35",
          life: 1,
          decay: 0.015,
        });
      }
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        // Combine all special patterns
        const allNewSpecialPatterns = [
          ...(newLineResult.specialPatterns || []),
          ...newPatternResult.patterns,
        ];
  ...
  createSpecialItemsFromPatterns(patterns, matches) {
    patterns.forEach((pattern) => {
      // Obtener la posición correcta según el tipo de patrón
      let row, col;
      if (pattern.centerPosition) {
        // Para líneas de 4 y 5+
        ({ row, col } = pattern.centerPosition);
      } else if (pattern.corner) {
        // Para patrones L
        ({ row, col } = pattern.corner);
      } else if (pattern.center) {
        // Para patrones T
        ({ row, col } = pattern.center);
      } else {
        console.warn("Patrón sin posición válida:", pattern);
        return;
      }

      if (
        pattern.type === "bomb_4_horizontal" ||
        pattern.type === "bomb_4_vertical"
      ) {
        // Crear item especial de gato para líneas de 4 DESPUÉS de eliminar los elementos
        setTimeout(() => {
  ...
  activateCatPower(catRow, catCol, targetRow, targetCol) {
    this.animating = true;
    ...
    // Crear partículas especiales de gato
    for (let i = 0; i < 30; i++) {
      this.particles.push({
        x: catCol * this.tileSize + this.tileSize / 2,
        y: catRow * this.tileSize + this.tileSize / 2,
    ...
    // Eliminar toda la fila del GATO
    for (let c = 0; c < this.gridSize; c++) {
      if (this.board[catRow][c] && this.board[catRow][c] !== null) {
        // Crear partículas para cada tile eliminado
        for (let i = 0; i < 8; i++) {
          this.particles.push({
            x: c * this.tileSize + this.tileSize / 2,
            ...
            vx: (Math.random() - 0.5) * 15,
            vy: (Math.random() - 0.5) * 15,
            color: this.colors[this.board[catRow][c].type] || "#ff6b35",
            life: 1,
            decay: 0.02,
          });
        }
        ...
        this.animations.push({
          type: "scaleOut",
          tile: this.board[catRow][c],
          progress: 0,
          duration: 200,
        });

        this.board[catRow][c] = null;
      }
    }

    // Eliminar toda la columna del GATO
    for (let r = 0; r < this.gridSize; r++) {
      if (this.board[r][catCol] && this.board[r][catCol] !== null) {
        // Crear partículas para cada tile eliminado
        for (let i = 0; i < 8; i++) {
          this.particles.push({
            x: catCol * this.tileSize + this.tileSize / 2,
            ...
            vx: (Math.random() - 0.5) * 15,
            vy: (Math.random() - 0.5) * 15,
            color: this.colors[this.board[r][catCol].type] || "#ff6b35",
            life: 1,
            decay: 0.02,
          });
        }
        ...
        this.animations.push({
          type: "scaleOut",
          tile: this.board[r][catCol],
          progress: 0,
          duration: 200,
        });

        this.board[r][catCol] = null;
      }
    }
  ...
  activateGhostPower(ghostRow, ghostCol, targetRow, targetCol) {
    this.animating = true;
    ...
    // Crear partículas especiales de fantasma
    for (let i = 0; i < 25; i++) {
      this.particles.push({
        x: ghostCol * this.tileSize + this.tileSize / 2,
        y: ghostRow * this.tileSize + this.tileSize / 2,
    ...
    // El fantasma elimina un área de 3x3 alrededor de su posición
    for (let dr = -1; dr <= 1; dr++) {
      for (let dc = -1; dc <= 1; dc++) {
        ...
        if (r >= 0 && r < this.gridSize && c >= 0 && c < this.gridSize) {
          if (this.board[r][c] && this.board[r][c] !== null) {
            // Crear partículas para cada tile eliminado
            for (let i = 0; i < 6; i++) {
              this.particles.push({
                x: c * this.tileSize + this.tileSize / 2,
                ...
                vx: (Math.random() - 0.5) * 12,
                vy: (Math.random() - 0.5) * 12,
                color: this.colors[this.board[r][c].type] || "#ffffff",
                life: 1,
                decay: 0.018,
              });
            }
    ...
    // Actualizar puntuación (bonus por usar poder especial)
    this.score += 200; // Más puntos porque elimina un área
  ...
  gameOver() {
    ...
    const message =
      `¡Nivel ${this.currentLevel} no completado!\n\n` +
  ...
  showLevelCompleteModal(levelScore, movesBonus) {
    ...
    document.getElementById(
      "completedLevel"
    ).textContent = `Nivel ${this.currentLevel}: ${levelData.description}`;
    ...
    if (movesBonus > 0) {
      bonusInfo.innerHTML =
        `🎁 Bonus por movimientos restantes: +${movesBonus} puntos<br>` +
  ...
  updateAnimations(deltaTime) {
    this.animations = this.animations.filter((animation) => {
      ...
      if (animation.type === "scaleOut") {
        ...
        animation.tile.y =
          animation.tile.y +
          (animation.tile.targetY - animation.tile.y) * easeOut * 0.2;
        return progress < 1;
    ...
    for (let row = 0; row < this.gridSize; row++) {
      for (let col = 0; col < this.gridSize; col++) {
        if (this.board[row][col]) {
          ...
          // Obtener color apropiado para el tile
          let tileColor = this.colors[tile.type];
          if (!tileColor) {
            // Colores por defecto para items especiales
            if (tile.type === "special_cat") {
              tileColor = "#ff6b35";
            } else if (tile.type === "special_ghost") {
              tileColor = "#ffffff";
            } else if (tile.type === "special_cross") {
              tileColor = "#ffd700";
            } else {
              tileColor = "#ff6b35"; // Color por defecto
            }
          }

          gradient.addColorStop(0, tileColor);
          gradient.addColorStop(1, this.darkenColor(tileColor, 0.3));
          ...
          if (tile.isSpecial) {
            ...
            let glowColor = "#ff1744"; // Default
            if (tile.specialType === "cross") {
              glowColor = "#ffd700";
            } else if (tile.specialType === "cat") {
              glowColor = "#ff6b35";
            } else if (tile.specialType === "ghost") {
              glowColor = "#ffffff";
            }
